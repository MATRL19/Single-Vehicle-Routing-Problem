...
def epsilonGreedy(epsilon, s, Q, A, w, fn_approx, x_coordinates, y_coordinates, Matrix_Distance):
    ...
    if fn_approx == "ANN":
        ...
        model = MLPRegressor(activation='relu', solver='lbfgs', random_state=42, 
                            hidden_layer_sizes=(binNumber*binNumber, numOfTilings*binNumber, binNumber))
    ...
    if p > epsilon:
        ...
        for i in A[:, 0].tolist():
            ...
            if fn_approx == "linear":
                if Matrix_Distance[s][i] + np.array(w).reshape(1, numOfTilings*binNumber*binNumber*binNumber) @ np.array(x).reshape(1, numOfTilings*binNumber*binNumber*binNumber).T > MAX:
                    MAX = Matrix_Distance[s][i] + np.array(w).reshape(1, numOfTilings*binNumber*binNumber*binNumber) @ np.array(x).reshape(1, numOfTilings*binNumber*binNumber*binNumber).T
                    a = i
            if fn_approx == "ANN":
    ...
    def iteration(self, episodeNumber):
        ...
        for j in range(episodeNumber):
            ...
            while ~self.terminationTheTrajectory():
                ...
                if j < 0:
                    delta = (Matrix_Distance[st][act] + self.gamma*np.array(w).reshape(1, numOfTilings*binNumber*binNumber*binNumber) @ np.array(a1).reshape(1, numOfTilings*binNumber*binNumber*binNumber).T \
                                    - np.array(w).reshape(1, numOfTilings*binNumber*binNumber*binNumber) @ np.array(s).reshape(1, numOfTilings*binNumber*binNumber*binNumber).T)
                else:
            ...
            pd.DataFrame(record_data).to_csv("training.csv", mode='a', header=None, index=None)
        ...
        pathTest, dirs, filesTest = next(os.walk("Test/"))
        pathoptions = []

        for k in range(len(filesTest)):
            fullPath = pathTest+filesTest[k]
            pathoptions.append(fullPath)

        path = random.choice(pathoptions)
...
numOfTilings = 4
binNumber = 10
w = [random.uniform(-0.001, 0.001)] * np.ones([numOfTilings, binNumber*binNumber*binNumber])
pd.DataFrame(w).to_csv("weight.csv", mode='w', header=None, index=None)
w = pd.read_csv('weight.csv', index_col=False, header=None)
w = np.array(w)
tracking = np.zeros((len(Matrix_Distance), len(Matrix_Distance)))
pathTr, dirs, filesTr = next(os.walk("Train/"))
epoques = 10
